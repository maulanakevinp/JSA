            ...
            <div class="card-body">
                <form class="formPekerjaanUpdate" autocomplete="off" action="{{ route('langkahPekerjaan.update', $langkahPekerjaan) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control form-control-alternative" type="datetime-local" name="waktu" id="waktu" placeholder="Masukkan Waktu ..." value="{{ old('waktu', date('Y-m-d\TH:i',strtotime($langkahPekerjaan->waktu))) }}">
                        @error('waktu')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
